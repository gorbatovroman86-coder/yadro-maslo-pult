...
function calcModel(cfg) {
  ...
  for (var m = 0; m < h.months; m++) {
    /* --- решение о культуре месяца: только на 1-е число ---
       ...
       Запускаемся на ядре 2 кат., если доступного хватает на полный месяц работы
    ...
    if (isLast) {
      ...
      /* месяц на ядре 2 кат.: страховой рапс на свободное место складов.
         Свободное место = потолок минус ядро на 1-е число (в такой месяц ядро только убывает). */
