...
var HEAD = '> Посчитано при: заход семечки **' + f(P.kernIntake) + ' т/сут**, маслоцех **' +
...
say(HEAD + '\n');
say('| Вариант | Ядро 2 кат. | Рапс | Закуп рапса | Закуп семечки | Продано П/Ф | Пик хран. | Капитал | Фин. результат | Рент. |');
say('|---|---|---|---|---|---|---|---|---|---|');
...
/* ---- 6б. мощность маслоцеха по ядру ---- */
say('\n### Мощность маслоцеха по ядру\n');
say(HEAD + '\n');
say('Приход ядра 2 кат. — ' + (CONFIG.kernel.intake.v * CONFIG.kernel.yKern2.v).toFixed(2) +
  ' т/сут. Под каждую мощность остановка обрушки подобрана заново, чтобы остаток вышел в ноль.\n');
say('| Маслоцех, т/сут | Ядро 2 кат. | Рапс | Закуп рапса | Стоп, сут | Недозагрузка обрушки | Пик, т | Фин. результат | Рент. |');
say('|---|---|---|---|---|---|---|---|---|');
[60, 60.3, 70, 80, 90].forEach(function (v) {
  var t = tuneStop({ 'oil.intakeKern': v }), k = t.k;
  say('| ' + r2(v) + ' | ' + k.kernMonths + ' | ' + k.rapeMonths + ' | ' + f(k.rapeBuy) + ' т | ' + t.stop +
    ' | ' + f(t.stop * CONFIG.kernel.intake.v) + ' т | ' + f(k.peakStock) + ' | **' + M(k.profit) + ' млн** | ' +
    (k.profit / k.revenue * 100).toFixed(1) + ' % |');
});

/* ---- 6в. заход семечки ---- */
say('\n### Чувствительность к заходу семечки\n');
say(HEAD + '\n');
say('| Заход семечки, т/сут | Стоп, сут | Ядро 2 кат. | Рапс | Закуп рапса | Закуп семечки | Пик, т | Сверх, т | Фин. результат | Рент. |');
say('|---|---|---|---|---|---|---|---|---|---|');
[90, 105, 120, 134, 140].forEach(function (v) {
  var t = tuneStop({ 'kernel.intake': v }), k = t.k;
  say('| ' + f(v) + ' | ' + t.stop + ' | ' + k.kernMonths + ' | ' + k.rapeMonths + ' | ' + f(k.rapeBuy) +
    ' т | ' + f(k.seedBuy) + ' т | ' + f(k.peakStock) + ' | ' + f(k.overPeak) + ' | **' + M(k.profit) +
    ' млн** | ' + (k.profit / k.revenue * 100).toFixed(1) + ' % |');
});
